...
switch lower(ext)
  case '.mat'
    ...
    if isfield(S,'baseline'), baseline = S.baseline; end
    if isfield(S,'TR'),       TR = S.TR; end
...
if QC.rejPct > 0
    choice = questdlg( ...
        sprintf(['Frame-rate QC detected %.1f%% unstable volumes.\n\n' ...
                 'Do you want to INTERPOLATE these volumes for VIDEO ONLY?\n\n' ...
                 '• YES  = smoother video (Urban-style interpolation)\n' ...
                 '• NO   = keep raw data\n\n' ...
                 'NOTE: Original data is never modified.'], QC.rejPct), ...
        'Frame-rate rejection (QC)', ...
        'Yes (interpolate)','No (keep raw)','No (keep raw)');

    applyRejection = strcmp(choice,'Yes (interpolate)');
end
if applyRejection
    fprintf('Applying frame-rate rejection (video-only interpolation)...\n');
    I = interpolateRejectedVolumes(I, QC.outliers);
else
    fprintf('Frame-rate rejection skipped. Using raw data.\n');
end
...
function QC = runFrameRateQC(I, TR)
% Diagnostic frame-rate / PNS QC (Urban / Montaldo inspired)
