...
rejectionMode  = 'none';   % default = NO interpolation

if QC.rejPct > 0

    choice = questdlg( ...
        sprintf(['Frame-rate QC detected %.1f%% unstable volumes.\n\n' ...
                 'Select interpolation mode (or Ignore to keep data unchanged):'], ...
                 QC.rejPct), ...
        'Frame-rate QC', ...
        'High spikes only', ...
        'Low dips only', ...
...
if applyRejection
    ...
    fprintf('[INFO] Interpolating %d / %d rejected volumes (video data only).\n', ...
        nnz(QC.outliers), numel(QC.outliers));

    % ---- Create interpolated dataset ONCE ----
    switch rejectionMode
    case 'high'
        rejMask = QC.globalNormSignal > QC.thresholdHigh;

    case 'low'
        rejMask = QC.globalNormSignal < QC.thresholdLow;

    otherwise   % 'both'
        rejMask = QC.outliers;
end

I_interp = interpolateRejectedVolumes(I, rejMask);


    % ---- Optional: QC on interpolated data (for PNGs) ----
    QC_interp = runFrameRateQC(I_interp, TR, 'INTERPOLATED', true);

    fprintf('[INFO] Interpolated QC completed.\n');

    % ---- IMPORTANT: replace I ONLY here ----
    I = I_interp;

else
    I_interp = I;   % safety: always defined
end

%% ===================== GUI =====================
play_fusi_video_final( ...
    I, I_interp, PSC, bg, par, initialFPS, maxFPS, TR, Tmax, baseline, ...
    loadedMask, loadedMaskIsInclude, nVols, applyRejection, QC);
...
% ==========================================================
% STEP 3 — SAFETY: ensure I_interp always exists in GUI scope
% ==========================================================
if isempty(I_interp)
    I_interp = I;
end
...
saveInterpBtn = uicontrol('Style','pushbutton','Units','pixels',...
...
function saveInterpolatedMat(~,~)
    ...
    % ================= DATA =================
    out.I = I_interp;   % interpolated data ONLY

% ================= METADATA =================
metadata = struct();

% -------------------------------------------------
% 1) Copy RAW acquisition metadata if present
% -------------------------------------------------
if exist('S','var') && isfield(S,'metadata') && isstruct(S.metadata)
    metadata = S.metadata;   % start from raw metadata
end

% -------------------------------------------------
% 2) Ensure acquisition timing matches THIS data
% -------------------------------------------------
metadata.TR   = TR;
...
% -------------------------------------------------
% 3) Acquisition geometry (if present in raw file)
% -------------------------------------------------
if exist('S','var')
...
% -------------------------------------------------
% 5) Baseline definition (your own script logic)
% -------------------------------------------------
...
k = 3;   % lower value = more strict. Higher value = less strict. 
thresholdHigh = 1 + k*sigma;
thresholdLow  = 1 - k*sigma;
outliers = (gNorm > thresholdHigh) | (gNorm < thresholdLow);

% -------------------------------------------------
% EXPAND rejection window (recommended)
% -------------------------------------------------
pad = 0;   % expand ±1 volume (use 2 if very noisy)
